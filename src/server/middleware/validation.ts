...
export const handleValidationErrors = (
  ...
  if (!errors.isEmpty()) {
    // Get the first error message for test compatibility
    const firstError = errors.array()[0];
    const fieldName = 'path' in firstError ? firstError.path : 'field';
    const errorMessage = `${fieldName}: ${firstError.msg}`;
...
export const validateUserRegistration = [
  ...
  body('role')
    .isIn(['resident', 'organizer'])
    .withMessage('Role must be resident or organizer'),
...
export const validateEventCreation = [
  ...
  body('ticketPrice')
    ...
    .isInt({ min: 100 })
    .withMessage('Ticket price must be at least ₹1 (100 paise)'),
  handleValidationErrors,
];
...
export const validateModerateReview = [
  body('action')
    ...
    .withMessage('Valid moderation action is required'),
  body('reason')
    .optional()
    .trim()
    .isLength({ max: 500 })
...
export const validateRefund = [
  ...
  body('amount')
    .isInt({ min: 100 })
    .withMessage('Refund amount must be at least ₹1 (100 paise)'),
  body('reason')
    .trim()
    .isLength({ min: 5, max: 200 })
    .withMessage('Reason must be between 5 and 200 characters'),
