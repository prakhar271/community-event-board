...
class BackgroundJobService {
  ...
  private async sendEventReminders(
    ...
    try {
      ...
      console.log(
        `📧 Processing reminders for ${upcomingEvents.length} events`
      );
      ...
      for (const event of upcomingEvents) {
        for (const registration of event.registrations) {
          try {
            ...
            console.log(
              `✅ Reminder sent to ${registration.user.email} for "${event.title}"`
            );
            ...
            console.error(
              `❌ Failed to send reminder to ${registration.user.email}:`,
              error
            );
          }
        }
  ...
  private async cleanupExpiredSessions(): Promise<void> {
    try {
      ...
      const expiredTokenCutoff = new Date(
        now.getTime() - 7 * 24 * 60 * 60 * 1000
      );
      console.log(
        `🗑️ Cleaning refresh tokens older than ${expiredTokenCutoff.toISOString()}`
      );

      // Clean up expired password reset tokens (older than 1 hour)
      const expiredResetCutoff = new Date(now.getTime() - 60 * 60 * 1000);
      console.log(
        `🔑 Cleaning password reset tokens older than ${expiredResetCutoff.toISOString()}`
      );

      // Clean up expired email verification tokens (older than 24 hours)
      const expiredVerificationCutoff = new Date(
        now.getTime() - 24 * 60 * 60 * 1000
      );
      console.log(
        `📧 Cleaning email verification tokens older than ${expiredVerificationCutoff.toISOString()}`
      );
  ...
  private async generateDailyAnalytics(_eventService: any): Promise<void> {
    try {
